...
public class JsonMqttAdaptor implements MqttTransportAdaptor {
  ...
  @Override
  public AdaptorToSessionActorMsg convertToActorMsg(SessionContext ctx, MsgType type, MqttMessage inbound)
  ...
  @Override
  public Optional<MqttMessage> convertToAdaptorMsg(SessionContext context, SessionActorToAdaptorMsg sessionMsg)
      throws AdaptorException {
    ...
    if (context instanceof DeviceSessionCtx) {
      ...
      case TO_DEVICE_RPC_REQUEST:
        ...
        result = createMqttPublishMsg(ctx, MqttTopics.THING_RPC_REQUESTS_TOPIC + rpcRequest.getRequestId(), rpcRequest);
        break;
  ...
  private MqttPublishMessage createMqttPublishMsg(SessionContext ctx, String topic, AttributesKVMsg msg,
      boolean asMap) {
  ...
  private FromDeviceMsg convertToGetAttributesRequest(SessionContext ctx, MqttPublishMessage inbound)
      throws AdaptorException {
  ...
  private FromDeviceMsg convertToRpcCommandResponse(SessionContext ctx, MqttPublishMessage inbound)
      throws AdaptorException {
    String topicName = inbound.variableHeader().topicName();
    try {
  ...
  private FromDeviceMsg convertToServerRpcRequest(SessionContext ctx, MqttPublishMessage inbound)
      throws AdaptorException {
    String topicName = inbound.variableHeader().topicName();
    String payload = validatePayload(ctx.getSessionId(), inbound.payload());
    try {
