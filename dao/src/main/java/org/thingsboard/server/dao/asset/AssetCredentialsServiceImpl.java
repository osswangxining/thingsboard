...
import static org.thingsboard.server.common.data.CacheConstants.ASSET_CREDENTIALS_CACHE;
...
public class AssetCredentialsServiceImpl implements AssetCredentialsService {
    ...
    @Override
    @Cacheable(cacheNames = ASSET_CREDENTIALS_CACHE, unless="#result == null")
    ...
    @Override
    @CacheEvict(cacheNames = ASSET_CREDENTIALS_CACHE, keyGenerator="previousAssetCredentialsId", beforeInvocation = true)
...
//        if (assetCredentials.getCredentialsType() == AssetCredentialsType.X509_CERTIFICATE) {
...
//        String cert = EncryptionUtil.trimNewLines(assetCredentials.getCredentialsValue());
...
//    }
    ...
    @Override
    @CacheEvict(cacheNames = ASSET_CREDENTIALS_CACHE, key="#assetCredentials.credentialsId")
    ...
    private DataValidator<AssetCredentials> credentialsValidator =
            new DataValidator<AssetCredentials>() {

                @Override
                protected void validateCreate(AssetCredentials assetCredentials) {
                    AssetCredentials existingCredentialsEntity = assetCredentialsDao.findByCredentialsId(assetCredentials.getCredentialsId());
                    ...
                    switch (assetCredentials.getCredentialsType()) {
                        case ACCESS_TOKEN:
                            if (assetCredentials.getCredentialsId().length() < 1 || assetCredentials.getCredentialsId().length() > 20) {
                                throw new DataValidationException("Incorrect access token length [" + assetCredentials.getCredentialsId().length() + "]!");
                            }
                            break;
